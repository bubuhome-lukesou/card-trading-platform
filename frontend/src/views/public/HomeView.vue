...
const getCategoryEmoji = (category: string) => categoryMap[category]?.emoji || '📦'
const getCategoryName = (category: string) => {
  const cat = categoryMap[category]
  if (!cat) return category
  return locale.value === 'zh' ? cat.zh : cat.en
}
          ...
          >
            ...
            <div class="listing-info">
              <div class="listing-category">
                <span class="listing-category-emoji">{{ getCategoryEmoji(item.category) }}</span>
                <span class="listing-category-name">{{ getCategoryName(item.category) }}</span>
              </div>
              <h3 class="listing-title">{{ getTitle(item) }}</h3>
              <div v-if="item.productTypeTag" class="listing-type-tag">
                <span class="type-dot"></span>
                {{ item.productTypeTag.name }}
              </div>
