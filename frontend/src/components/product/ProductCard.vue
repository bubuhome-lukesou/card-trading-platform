...
// Category display - show as emoji + name
const categoryMap: Record<string, { emoji: string; zh: string; en: string }> = {
  pokemon: { emoji: '🎮', zh: '寶可夢', en: 'Pokemon' },
  yugioh: { emoji: '🐉', zh: '遊戲王', en: 'Yu-Gi-Oh!' },
  mtg: { emoji: '🧙', zh: '萬智牌', en: 'MTG' },
  ultraman: { emoji: '👾', zh: '超人迪卡', en: 'Ultraman' },
  onepiece: { emoji: '⚔️', zh: '海賊王', en: 'One Piece' },
  doraemon: { emoji: '🤖', zh: '多啦A夢', en: 'Doraemon' },
  sports: { emoji: '⚽', zh: '運動', en: 'Sports' },
  other: { emoji: '🎴', zh: '其他', en: 'Other' }
}

const categoryInfo = computed(() => {
  const cat = categoryMap[props.product.category]
  if (!cat) return { emoji: '📦', label: props.product.category }
  return {
    emoji: cat.emoji,
    label: locale.value === 'zh' ? cat.zh : cat.en
  }
})

const productTypeTagName = computed(() => {
  if (!props.product.productTypeTag) return null
  return props.product.productTypeTag.name
})
...
<template>
  <RouterLink :to="`/product/${product.id}`" class="product-card">
    ...
    <div class="card-info">
      ...
      <!-- Product Type Tag (商品種類) -->
      <div v-if="productTypeTagName" class="product-type-tag">
        <span class="type-tag-dot"></span>
        <span class="type-tag-name">{{ productTypeTagName }}</span>
      </div>
