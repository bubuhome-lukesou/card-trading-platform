...
export class ProductsService {
  ...
  async findAll(filters: ProductFiltersDto) {
    const queryBuilder = this.productRepo
      ...
      .leftJoinAndSelect('product.productTypeTag', 'productTypeTag')
  ...
  async findOne(id: string): Promise<Product> {
    const product = await this.productRepo.findOne({
      where: { id },
      relations: ['seller', 'tags', 'productTypeTag']
    })
  ...
  async update(id: string, dto: UpdateProductDto, userId: string): Promise<Product> {
    console.log('[DEBUG update] dto received:', JSON.stringify(dto))
    console.log('[DEBUG update] dto.productTypeTagId:', dto.productTypeTagId)
    const product = await this.findOne(id)
    console.log('[DEBUG update] product loaded, productTypeTagId:', product.productTypeTagId, 'productTypeTag:', product.productTypeTag)
    ...
    // Handle productTypeTagId - use query builder to update FK column directly
    if (dto.productTypeTagId !== undefined) {
      // Use query builder for reliable FK column update
      if (dto.productTypeTagId && dto.productTypeTagId > 0) {
        const tag = await this.tagRepo.findOne({ where: { id: dto.productTypeTagId } })
        if (tag) {
          product.productTypeTag = tag
          product.productTypeTagId = dto.productTypeTagId
          await this.productRepo.createQueryBuilder()
            .update(Product)
            .set({ productTypeTagId: dto.productTypeTagId } as any)
            .where('id = :id', { id: product.id })
            .execute()
        }
      } else {
        product.productTypeTag = null as any
        await this.productRepo.createQueryBuilder()
          .update(Product)
          .set({ productTypeTagId: null as any } as any)
          .where('id = :id', { id: product.id })
          .execute()
      }
      delete (dto as any).productTypeTagId
    }

    Object.assign(product, dto)
    await this.productRepo.save(product)
    // Use raw SQL to update productTypeTagId column directly (bypass TypeORM)
    if (product.productTypeTagId !== undefined && product.productTypeTagId !== null) {
      await this.productRepo.query(`UPDATE products SET productTypeTagId = ${product.productTypeTagId} WHERE id = '${product.id}'`)
    } else {
      await this.productRepo.query(`UPDATE products SET productTypeTagId = NULL WHERE id = '${product.id}'`)
    }
    // Return product with correct productTypeTag loaded
    return this.productRepo.findOne({ where: { id: product.id }, relations: ['productTypeTag'] })
  }
